...
func (c *BasicSectionConf) Unmarshal(configObject interface{}, fileName ...string) aerror.Error {
	...
	if err != nil {
		return aerror.NewError(err, aerror.Code.BReadFileFailed, "read filed failed")
	}

	err = c.parse(configObject)
	if err != nil {
		return aerror.NewError(err, aerror.Code.BReadFileFailed, "parse file failed")
	}
...
func (c *BasicSectionConf) parse(configObject interface{}) error {
	...
	for i := 0; i < sv.NumField(); i++ {
		...
		if len(tagArr) != 2 {
			return aerror.NewErrorf(nil, aerror.Code.CParamsError, "config object tag must split by %s", tagSep)
		}
		...
		if !sv.Field(i).CanSet() {
			return aerror.NewErrorf(nil, aerror.Code.CParamsError, "struct filed: %s can not set", st.Field(i).Name)
		}
		...
		case reflect.Bool:
			...
			if err != nil {
				return aerror.NewErrorf(err, aerror.Code.CParamsError, "a bool-value is expected at config file line: %d", configValueItem.Line)
			}
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			...
			if err != nil {
				return aerror.NewErrorf(err, aerror.Code.CParamsError, "a int-value is expected at config file line: %d", configValueItem.Line)
			}
	...
	for rd.Scan() {
		...
		if strings.HasPrefix(row, SectionS) {
			if !strings.HasSuffix(row, sectionE) {
				return aerror.NewErrorf(err, aerror.Code.CParamsError, "no end section: %s, at: %d", sectionE, line)
			}
		...
		if len(sectionStr) == 0 {
			return aerror.NewErrorf(err, aerror.Code.CParamsError, "section lost at line: %d", line)
		}

		// 分析行的 key value
		kvArr := strings.Split(row, KVSep)
		if len(kvArr) != 2 {
			return aerror.NewErrorf(err, aerror.Code.CParamsError, "config line: %d is expected only two column", line)
		}
